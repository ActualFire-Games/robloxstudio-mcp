...
- get_services lists the common services with child counts, or one service by name; get_instance_children lists one level of children with hasChildren, hasSource, and enabled flags. Prefer get_project_structure for a bounded overview and get_descendants for a whole subtree. An "Unknown endpoint" error from any tool means the Studio plugin is older than the server: rebuild with npm run build:all and restart Studio.
