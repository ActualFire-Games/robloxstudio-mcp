...
describe('script source update safety', () => {
  ...
  test('setScriptSource returns the in-place failure without constructing a replacement', async () => {
    // FindFirstChildOfClass exists on every real Instance; the ancestry walk for the
    // package-modified warning calls it on each ancestor.
    const parent = { name: 'parent', FindFirstChildOfClass: () => undefined };
    const destroy = jest.fn();
    const original = {
      Name: 'Main',
      ...
      // Real Instances always expose this; the package-warning lookup walks ancestry with it.
