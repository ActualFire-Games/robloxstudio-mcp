...
recipes and a few consolidated tools. **This fork keeps them.** The table is kept
as a map of upstream-equivalent workflows, not as a list of things you can no
longer call here.

The one exception is the deprecated playtest aliases (`start_playtest`,
`stop_playtest`, `multiplayer_test_*`): those stay hidden from the advertised
catalog but remain callable, exactly as before.
...
In this fork these names remain in both `tools/list` and the direct
`/mcp/<tool>` compatibility routes, apart from the deprecated playtest aliases
noted above, which are routable but unadvertised.
