...
- Catch a spike as it happens with `capture_micro_profiler` triggered captures: `action:"arm"` with a frame-time, attribute, or log trigger, then poll `action:"collect"`. `frame_breakdown` reports per-frame top timers for the longest frames and the trigger frame, and `action:"analyze"` re-slices a stored snapshot without re-capturing.
